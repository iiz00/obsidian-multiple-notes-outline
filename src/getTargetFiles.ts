...
export function getOutgoingLinkFiles(
	app: App,
	file: TFile,
	info: FileInfo,
	cache: OutlineData[],
	...
	for (let i = 0; i < info?.frontmatterLinks?.length; i++) {
		const fileobj = app.metadataCache.getFirstLinkpathDest(
			info.frontmatterLinks[i].link,
			file.path,
		);
...
export function getBacklinkFilesDataview(
	app: App,
	file: TFile,
	...
	const backlinks = app.plugins.plugins.dataview?.api?.pages(`"${file.path}"`)?.values[0]?.file
		.inlinks.values;
	if (!backlinks) {
		return getBacklinkFiles(app, file);
	}
