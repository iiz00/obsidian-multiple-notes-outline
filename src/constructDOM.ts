...
export function constructNoteDOM(
	...
	for (let i = 0; i < files.length; i++) {
		...
		const belongsAOT =
			Boolean(
				status[si].isTop == true && !Object.values(status[si].duplicated).includes(true),
			) && category != "main";
		...
		const noteChildrenEl: HTMLElement = noteEl.createDiv(
...
export function constructOutlineDOM(
		...
		frontmatterlinksloop: for (let j = 0; j < info.frontmatterLinks.length; j++) {
			...
			if (this.settings.hideLinksBetweenRelatedFiles == "all") {
				for (const category in this.targetFiles) {
					...
					if (
						this.targetFiles[category].some(
							(targetfile) => targetfile.path == linkTarget.path,
						)
					) {
			...
			const outlineTitle: HTMLElement = outlineEl.createDiv(
			...
			outlineTitle.addEventListener("mouseover", (event: MouseEvent) => {
				if (linkTarget) {
					...
					let posInfo = {};
					if (linkSubpath) {
						const subpathPosition = getSubpathPosition(
							this.app,
							linkTarget,
							linkSubpath,
						);
		...
		elementloop: for (let j = 0; j < data.length; j++) {
			...
			const linkTarget =
				element !== "link"
					...
					: this.app.metadataCache.getFirstLinkpathDest(
							parseLinktext(data[j]?.link).path,
							file.path,
						);
			...
			if (element == "link") {
				if (this.settings.hideLinksBetweenRelatedFiles == "mainOnly") {
					...
					if (
						this.app.metadataCache.getFirstLinkpathDest(data[j].link, file.path)
							?.path == this.targetFiles.main?.[0].path
					) {
						continue;
					}
				}
			...
			if (element != "heading" && this.settings.indentFollowHeading) {
				const additionalIndent =
					(latestHeadingLevel -
						(this.maxLevel + 1) +
						(this.settings.indentFollowHeading == 2 ? 1 : 0)) *
					1.5;
			...
			if (
				!isCanvas &&
				element == "link" &&
				data[j].position.start.line == data[j - 1]?.position.start.line
			) {
				indent = indent + 1.5;
			}
			...
			if (element == "listItems" && data[j].task !== void 0) {
				prefix =
					data[j].task == "x" ? this.settings.prefix.taskDone : this.settings.prefix.task;
				if (this.settings.addCheckboxText) {
					prefix = prefix + "[" + data[j].task + "] ";
				}
			}

			const outlineTitleContent = outlineTitle.createDiv(
			...
			if (!isCanvas && this.settings.inlinePreview) {
				...
				if (
					...
					previewText = info.lines[data[j].position.start.line].slice(
						data[j].position.end.col,
					);
				} else {
					previewText =
			...
			if (this.settings.tooltipPreview) {
				if (!isCanvas) {
					...
					endpreviewloop: while (k < data.length) {
						...
						if (this.settings.showElements[data[k].typeOfElement]) {
							...
							if (
								...
								(data[k].level >= 2 ||
									(this.settings.allRootItems == false &&
										data[k].level == 1 &&
										(this.settings.allTasks == false ||
											data[k].task === void 0)) ||
								...
								for (const value of this.settings.wordsToIgnore[
									data[k].typeOfElement
								]) {
					...
					if (data[j].typeOfElement == "listItems") {
						...
						outlineTitle.dataset.tooltipPosition =
							this.settings.tooltipPreviewDirection;
						outlineTitle.setAttribute("data-tooltip-delay", "10");
					}
				}
			}
			...
			outlineTitle.addEventListener(
				...
				async (event: MouseEvent) => {
					if (this.settings.openLinkByClick == true && element == "link") {
						...
						scrollToElement(
							data[j].position.start.line,
							data[j].position.start.col,
							this.app,
						);
					}
				},
						...
						item
							...
							.onClick(async () => {
								...
								this.app.internalPlugins.plugins[
									"global-search"
								]?.instance.openGlobalSearch(searchString);
							}),
					);
					menu.addSeparator();
				}
	...
	backlinkfileloop: for (let i = 0; i < info.backlinks?.length; i++) {
		...
		outlineTitle
			.createDiv("tree-item-inner nav-file-title-content")
			.setText(info.backlinks[i].basename);
		...
		switch (displayFileInfo) {
			...
			case "tag":
				if (status.outlineReady) {
					const firsttagIndex = data.findIndex(
						(element, index) => data[index].typeOfElement == "tag",
					);
					if (firsttagIndex >= 0) {
