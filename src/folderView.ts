import { debounce, Debouncer, Menu, TFolder } from "obsidian";
...
export class MultipleNotesOutlineFolderView extends ItemView {
		...
		if (this.flagChanged && !this.flagRegetTarget) {
			for (let i = 0; i < this.changedFiles.length; i++) {
				for (const folder in this.targetFiles) {
					...
					this.fileInfo[folder][index] = await getFileInfo(
						this.app,
						this.targetFiles[folder][index] as TFile,
						this.settings,
						false,
					...
					const newData = await getOutline(
						this.app,
						this.targetFiles[folder][index] as TFile,
						this.fileStatus[folder][index],
						this.fileInfo[folder][index],
						this.settings,
					);
					if (newData) {
					...
					const updateNoteChildrenEl = document.getElementById(
						"MNO" + this.viewType + this.targetFiles[folder][index].path,
					);
					updateNoteChildrenEl.empty();
					constructOutlineDOM.call(
						this,
						this.targetFiles[folder][index],
						this.fileInfo[folder][index],
						this.outlineData[folder][index],
	...
	async refreshView(flagGetTarget: boolean, flagGetOutline: boolean) {
		...
		if (this.settings.showDebugInfo) {
			console.log(
				"Multiple Notes Outline: time required to get outlines, folder view: ",
		...
		if (this.settings.showDebugInfo) {
			console.log(
				"Multiple Notes Outline: time required to draw outlines, folder view: ",
				this.targetFolder.path,
				endTime - midTime,
			);
			console.log(
				"Multiple Notes Outline: time required to refresh view, folder view: ",
				this.targetFolder.path,
				endTime - startTime,
			);
		}
	...
	private drawOutline(previousY: number): void {
		...
		const folderTitleEl: HTMLElement = folderEl.createDiv(
		...
		constructNoteDOM.call(
			this,
			this.targetFiles[this.targetFolder.path],
			this.fileStatus[this.targetFolder.path],
			this.fileInfo[this.targetFolder.path],
			this.outlineData[this.targetFolder.path],
			categoryMainEl,
			"folder",
			categoryAOTEl,
			this.targetFolder,
			this.fileOrder[this.targetFolder.path],
		);
